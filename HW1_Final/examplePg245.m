...
Orbital_Elements - orbital elements [h e RA incl w TA a]
...
%...
%...Algorithm 5.2:
[v1, v2] = my_lambert(r1, r2, dt);
%...Algorithm 4.1 (using r1 and v1):
Orbital_Elements = r_v_2_O_E(r1, v1, mu);
%...Save the initial true anomaly:
TA1 = Orbital_Elements(6);
%...Algorithm 4.1 (using r2 and v2):
Orbital_Elements = r_v_2_O_E(r2, v2, mu);
%...Save the final true anomaly:
TA2 = Orbital_Elements(6);
...
v2(1), v2(2), v2(3))
fprintf('\n\n Orbital elements:')
fprintf('\n Angular momentum (km^2/s) = %g', Orbital_Elements(1))
fprintf('\n Eccentricity = %g', Orbital_Elements(2))
fprintf('\n Inclination (deg) = %g', Orbital_Elements(4))
fprintf('\n RA of ascending node (deg) = %g', Orbital_Elements(3))
fprintf('\n Argument of perigee (deg) = %g', Orbital_Elements(5))
fprintf('\n True anomaly initial (deg) = %g', TA1)
fprintf('\n True anomaly final (deg) = %g', TA2)
fprintf('\n Semimajor axis (km) = %g', Orbital_Elements(7))
fprintf('\n Periapse radius (km) = %g', Orbital_Elements(1)^2/mu/(1 + Orbital_Elements(2)))
%...If the orbit is an ellipse, output its period:
if Orbital_Elements(2)<1
    T = 2*pi/sqrt(mu)*Orbital_Elements(7)^1.5;
fprintf('\n Period:')
fprintf('\n Seconds = %g', T)
fprintf('\n Minutes = %g', T/60)
fprintf('\n Hours = %g', T/3600)
