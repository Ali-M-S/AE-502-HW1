...
max_v1 = 60; % (km/s) maximum delta v for rendez-vous
max_v2 = 20; % (km/s) maximum delta v for fly-by

[err,data1] = LoadData('C:\Users\Asus\Desktop\AE 502 Advanced Orbital Mechanics\HW\HW1\Actual\Earth2Borisov.txt');
...
[err,data2] = LoadData('C:\Users\Asus\Desktop\AE 502 Advanced Orbital Mechanics\HW\HW1\Actual\Borisov_arrival.txt');
...
fprintf('\n departure = %d/1308 | arrival= %d/976', nd, na)
      ...
      [v_depart_short, v_arrive_short] = my_lambert(re(nd,:), rcom(na,:), tof_seconds); %Solve lambert short transfer problem
        ...
        % departing velocity - earth velocity (Leaving earth)
         ...
         % arrival velocity - commet velocity (arriving to comet)
...
title('Rendevouz Extremal Field Map (Earth to Borisov)');
xlabel('Departure Days after January 1st 2017');
ylabel('Arrival Days after June 1st 2019');
...
title('Rendevouz Porkchop plot (Earth to Borisov)');
xlabel('Earth departure date');
ylabel('Borisov arrival date');
...
title('Fly-by Extremal Field Map (Earth to Borisov)');
xlabel('Departure Days after January 1st 2017');
ylabel('Arrival Days after June 1st 2019');
legend('Total Δv (km/s)', 'Location', 'best');
hold off
...
title('Fly-by Porkchop Plot (Earth to Borisov)');
xlabel('Earth departure date');
ylabel('Borisov arrival date');
